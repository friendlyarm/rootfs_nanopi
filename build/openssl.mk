...
$(PKG_NAME)/.config: $(PKG_NAME)
	$(SILENT)if [ ! -f $@ ]; then \
	  cd $(PKG_NAME) && { \
	    CC=$(CROSS_COMPILE)gcc AR=$(CROSS_COMPILE)ar RANLIB=$(CROSS_COMPILE)ranlib ./Configure threads shared zlib --prefix=${PREFIX} --openssldir=${PREFIX}/openssl linux-armv4 -march=armv5te ; \
	    touch .config; \
	  } ;\
...
install_lib: build
	$(SILENT)$(MAKE) -C $(PKG_NAME) CROSS_COMPILE= INSTALL_PREFIX=${EXTLIB_DIR} install_sw
